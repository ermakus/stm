...
- (void)attemptCrashReportSubmission
{
	...
	if (![self _isSubmissionHostReachable]) {
		...
		if (![[NSUserDefaults standardUserDefaults] boolForKey: kAutomaticallySendCrashReports]) {
			UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Oops.. Crash log found."
						message:@"It looks like BitTorrent plugin crashed last time it used. Do you want to send an error report to help fix this issue?"
						   delegate:self
...
- (void)_sendCrashReports
{
	...
	for (i=0; i < [_crashFiles count]; i++)
	{
		...
		if ([crashData length] > 0)
		{
			...
			[self _postXML:crashLogString toURL:_submissionURL];
		}
	}
...
- (NSString *)_crashLogStringForReport:(PLCrashReport *)report
{
	...
	[xmlString appendFormat:@"Exception Type:  %s\n", [report.signalInfo.name UTF8String]];
        [xmlString appendFormat:@"Exception Codes: %@ at 0x%x\n", report.signalInfo.code, report.signalInfo.address];
    ...
    for (PLCrashReportThreadInfo *thread in report.threads) {
        ...
        NSUInteger frame_idx;
        for (frame_idx = 0; frame_idx < [thread.stackFrames count]; frame_idx++) {
            ...
            [xmlString appendFormat: @"%-4ld%-36s0x%08x 0x%qx + %qd\n", 
             (long) frame_idx, [imageName UTF8String], frameInfo.instructionPointer, baseAddress >> 32, pcOffset >> 32];
        }
        [xmlString appendString: @"\n"];
    }
    ...
    for (PLCrashReportBinaryImageInfo *imageInfo in report.images) {
		...
		[xmlString appendFormat:@"0x%08qx - 0x%08qx  %@ %@ <%@> %@\n",
...
- (void)_postXML:(NSString*)xml toURL:(NSURL*)url
{
	...
	[postBody appendData:[[NSString stringWithFormat:@"id=%@&", [self urlEncodeValue:uniqueIdentifier ]] dataUsingEncoding:NSUTF8StringEncoding]];
	[postBody appendData:[[NSString stringWithFormat:@"xmlstring=%@", [self urlEncodeValue:xml ]] dataUsingEncoding:NSUTF8StringEncoding]];
