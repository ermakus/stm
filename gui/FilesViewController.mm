...
- (void)loadView {
   [super loadView];
   self.tabBarItem.image = load_image(@"files.png");
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return self.torrent->getFileCount();
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *CellIdentifier = @"CellFile";
    ...
    const char* fileName = self.torrent->getFileName( index );
    const char* name = strchr( fileName, '/' );
    if( name ) fileName = name+1;
    cell.textLabel.text = [NSString stringWithUTF8String: fileName];
