...
recv_peermsg( btContext *ctx, btPeer *peer) {
    ...
    bt_assert(peer->download<ctx->downloadcount);
    ...
    bt_assert(len <= MAXMESSAGE && len >= 0);
    err = kStream_fread( &peer->ios, nmsg, len + sizeof(int32_t));
    if (err < 0) goto cleanup;
    bt_assert( err == len + (int)sizeof(int32_t));
	...
	case BT_MSG_REQUEST:
		...
		if (!bs_isSet(&peer->blocks, piece) && 
		    ...
		    //printf("%d: Choked by invalid request for block %d (%s have it)\n", peer->ios.fd, piece, bs_isSet(&peer->blocks, piece)?"they":"we don't");
		}
	    }
	    break;
	case BT_MSG_PIECE:
	...
	case BT_MSG_EXTENDED:
		...
		if (bts_rewind(messageStream, BTS_INPUT))
		{
			bts_destroy(messageStream);
			return -1;
		}

		if (benc_get_object(messageStream, &messageDictionary)) 
		{
			bts_destroy(messageStream);
			return -2;
		}
		...
		if (nmsg[5] == 0)
		{
		    ...
		    uint16_t peerlistenport = 0;
                    ...
                    if( !btObject_val(messageDictionary, "p") ) return -2;
...
cleanup:
	...
	if (err == sizeof(nbo_len)) {
	    ...
	    bt_assert(tlen <= MAXMESSAGE && tlen >= 0);
...
void peer_shutdown( btContext *ctx, btPeer *peer, const char *error) {
    int fd = peer->ios.fd;
...
update_interested( btContext *ctx, btPeer *p) {
    ...
    bt_assert (p->download<ctx->downloadcount);
